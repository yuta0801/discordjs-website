...
export default {
  ...
  methods: {
    ...
    async loadMessages() {
      const [lang, repo, tag] = [this.lang, this.source.repo, this.tag];
      console.log('Loading messages', lang.name);
      const json = res => {
      ...
      const url = `/master/content/${lang.path}/docs/${repo.split('/')[1]}/${tag}.json`;
